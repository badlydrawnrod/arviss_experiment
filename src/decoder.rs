use super::{
    cpu_types::{DecodeRv32c, DecodeRv32f, DecodeRv32i, DecodeRv32m},
    tobits::ToBits,
};

pub trait Rv32iDecoder {
    type Item;

    fn decode_rv32i(&mut self, code: u32) -> Self::Item;
}

pub trait Rv32imDecoder {
    type Item;

    fn decode_rv32im(&mut self, code: u32) -> Self::Item;
}

pub trait Rv32imcDecoder {
    type Item;

    fn decode_rv32imc(&mut self, code: u32) -> Self::Item;
}

pub trait Rv32imfDecoder {
    type Item;

    fn decode_rv32imf(&mut self, code: u32) -> Self::Item;
}

impl<T, U> Rv32iDecoder for T
where
    T: DecodeRv32i + DecodeRv32i<Item = U>,
{
    type Item = U;

    // This function is generated by make_self.py. Do not edit.
    fn decode_rv32i(&mut self, code: u32) -> Self::Item {
...
impl<T, U> Rv32imDecoder for T
where
    T: DecodeRv32i + DecodeRv32i<Item = U> + DecodeRv32m + DecodeRv32m<Item = U>,
{
    type Item = U;
        ...
        + DecodeRv32m<Item = U>,
{
    ...
    match code & 0xfff0707f {
        0xe0000053 => return decoder.fmv_x_w(c.rd(), c.rs1()),
        0xe0001053 => return decoder.fclass_s(c.rd(), c.rs1()),
        0xf0000053 => return decoder.fmv_w_x(c.rd(), c.rs1()),
        _ => {}
    }
    match code & 0xfff0007f {
        0x58000053 => return decoder.fsqrt_s(c.rd(), c.rs1(), c.rm()),
        0xc0000053 => return decoder.fcvt_w_s(c.rd(), c.rs1(), c.rm()),
        0xc0100053 => return decoder.fcvt_wu_s(c.rd(), c.rs1(), c.rm()),
        0xd0000053 => return decoder.fcvt_s_w(c.rd(), c.rs1(), c.rm()),
        0xd0100053 => return decoder.fcvt_s_wu(c.rd(), c.rs1(), c.rm()),
        _ => {}
    }
    match code & 0xfe00707f {
    ...
    match code & 0xfe00707f {
        0x20000053 => return decoder.fsgnj_s(c.rd(), c.rs1(), c.rs2()),
        0x20001053 => return decoder.fsgnjn_s(c.rd(), c.rs1(), c.rs2()),
        0x20002053 => return decoder.fsgnjx_s(c.rd(), c.rs1(), c.rs2()),
        0x28000053 => return decoder.fmin_s(c.rd(), c.rs1(), c.rs2()),
        0x28001053 => return decoder.fmax_s(c.rd(), c.rs1(), c.rs2()),
        0xa0000053 => return decoder.fle_s(c.rd(), c.rs1(), c.rs2()),
        0xa0001053 => return decoder.flt_s(c.rd(), c.rs1(), c.rs2()),
        0xa0002053 => return decoder.feq_s(c.rd(), c.rs1(), c.rs2()),
        _ => {}
    }
    match code & 0x0000ffff {
    ...
    match code & 0xfe00007f {
        0x00000053 => return decoder.fadd_s(c.rd(), c.rs1(), c.rs2(), c.rm()),
        0x08000053 => return decoder.fsub_s(c.rd(), c.rs1(), c.rs2(), c.rm()),
        0x10000053 => return decoder.fmul_s(c.rd(), c.rs1(), c.rs2(), c.rm()),
        0x18000053 => return decoder.fdiv_s(c.rd(), c.rs1(), c.rs2(), c.rm()),
        _ => {}
    }
    match code & 0x0000f07f {
    ...
    match code & 0x0000707f {
        ...
        0x00002007 => return decoder.flw(c.rd(), c.rs1(), c.iimmediate()),
        0x00002027 => return decoder.fsw(c.rs1(), c.rs2(), c.simmediate()),
        _ => {}
    }
    match code & 0x0000ef83 {
    ...
    match code & 0x0600007f {
        0x00000043 => return decoder.fmadd_s(c.rd(), c.rs1(), c.rs2(), c.rs3(), c.rm()),
        0x00000047 => return decoder.fmsub_s(c.rd(), c.rs1(), c.rs2(), c.rs3(), c.rm()),
        0x0000004b => return decoder.fnmsub_s(c.rd(), c.rs1(), c.rs2(), c.rs3(), c.rm()),
        0x0000004f => return decoder.fnmadd_s(c.rd(), c.rs1(), c.rs2(), c.rs3(), c.rm()),
        _ => {}
    }
    match code & 0x0000007f {
...
#[cfg(test)]
mod test {
    use crate::DecodeRv32c;
    use crate::DecodeRv32f;
    use crate::DecodeRv32i;
    use crate::DecodeRv32m;

    use crate::Rv32iDecoder;
    use crate::Rv32imDecoder;
    use crate::Rv32imcDecoder;
    use crate::Rv32imfDecoder;
    ...
    impl DecodeRv32i for Checker {
        ...
        fn beq(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _bimm: u32) -> Self::Item {
            "beq"
        }

        fn bne(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _bimm: u32) -> Self::Item {
            "bne"
        }

        fn blt(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _bimm: u32) -> Self::Item {
            "blt"
        }

        fn bge(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _bimm: u32) -> Self::Item {
            "bge"
        }

        fn bltu(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _bimm: u32) -> Self::Item {
            "bltu"
        }

        fn bgeu(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _bimm: u32) -> Self::Item {
            "bgeu"
        }

        fn lb(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "lb"
        }

        fn lh(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "lh"
        }

        fn lw(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "lw"
        }

        fn lbu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "lbu"
        }

        fn lhu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "lhu"
        }

        fn addi(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "addi"
        }

        fn slti(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "slti"
        }

        fn sltiu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "sltiu"
        }

        fn xori(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "xori"
        }

        fn ori(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "ori"
        }

        fn andi(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "andi"
        }

        fn jalr(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "jalr"
        }

        fn sb(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _simm: u32) -> Self::Item {
            "sb"
        }

        fn sh(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _simm: u32) -> Self::Item {
            "sh"
        }

        fn sw(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _simm: u32) -> Self::Item {
            "sw"
        }

        fn auipc(&mut self, _rd: crate::Reg, _uimm: u32) -> Self::Item {
            "auipc"
        }

        fn lui(&mut self, _rd: crate::Reg, _uimm: u32) -> Self::Item {
            "lui"
        }

        fn jal(&mut self, _rd: crate::Reg, _jimm: u32) -> Self::Item {
            "jal"
        }

        fn add(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "add"
        }

        fn sub(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "sub"
        }

        fn sll(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "sll"
        }

        fn slt(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "slt"
        }

        fn sltu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "sltu"
        }

        fn xor(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "xor"
        }

        fn srl(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "srl"
        }

        fn sra(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "sra"
        }

        fn or(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "or"
        }

        fn and(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "and"
        }

        fn slli(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _shamt: u32) -> Self::Item {
            "slli"
        }

        fn srli(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _shamt: u32) -> Self::Item {
            "srli"
        }

        fn srai(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _shamt: u32) -> Self::Item {
            "srai"
        }

        fn fence(&mut self, _fm: u32, _rd: crate::Reg, _rs1: crate::Reg) -> Self::Item {
            "fence"
        }
    ...
    fn decode_rv32i() {
        let mut c = Checker {};
        assert_eq!("add", c.decode_rv32i(enc(MATCH_ADD, MASK_ADD)));
        assert_eq!("addi", c.decode_rv32i(enc(MATCH_ADDI, MASK_ADDI)));
        assert_eq!("and", c.decode_rv32i(enc(MATCH_AND, MASK_AND)));
        assert_eq!("andi", c.decode_rv32i(enc(MATCH_ANDI, MASK_ANDI)));
        assert_eq!("auipc", c.decode_rv32i(enc(MATCH_AUIPC, MASK_AUIPC)));
        assert_eq!("beq", c.decode_rv32i(enc(MATCH_BEQ, MASK_BEQ)));
        assert_eq!("bge", c.decode_rv32i(enc(MATCH_BGE, MASK_BGE)));
        assert_eq!("bgeu", c.decode_rv32i(enc(MATCH_BGEU, MASK_BGEU)));
        assert_eq!("blt", c.decode_rv32i(enc(MATCH_BLT, MASK_BLT)));
        assert_eq!("bltu", c.decode_rv32i(enc(MATCH_BLTU, MASK_BLTU)));
        assert_eq!("bne", c.decode_rv32i(enc(MATCH_BNE, MASK_BNE)));
        assert_eq!("ebreak", c.decode_rv32i(enc(MATCH_EBREAK, MASK_EBREAK)));
        assert_eq!("ecall", c.decode_rv32i(enc(MATCH_ECALL, MASK_ECALL)));
        assert_eq!("fence", c.decode_rv32i(enc(MATCH_FENCE, MASK_FENCE)));
        assert_eq!("jal", c.decode_rv32i(enc(MATCH_JAL, MASK_JAL)));
        assert_eq!("jalr", c.decode_rv32i(enc(MATCH_JALR, MASK_JALR)));
        assert_eq!("lb", c.decode_rv32i(enc(MATCH_LB, MASK_LB)));
        assert_eq!("lbu", c.decode_rv32i(enc(MATCH_LBU, MASK_LBU)));
        assert_eq!("lh", c.decode_rv32i(enc(MATCH_LH, MASK_LH)));
        assert_eq!("lhu", c.decode_rv32i(enc(MATCH_LHU, MASK_LHU)));
        assert_eq!("lui", c.decode_rv32i(enc(MATCH_LUI, MASK_LUI)));
        assert_eq!("lw", c.decode_rv32i(enc(MATCH_LW, MASK_LW)));
        assert_eq!("or", c.decode_rv32i(enc(MATCH_OR, MASK_OR)));
        assert_eq!("ori", c.decode_rv32i(enc(MATCH_ORI, MASK_ORI)));
        assert_eq!("sb", c.decode_rv32i(enc(MATCH_SB, MASK_SB)));
        assert_eq!("sh", c.decode_rv32i(enc(MATCH_SH, MASK_SH)));
        assert_eq!("sll", c.decode_rv32i(enc(MATCH_SLL, MASK_SLL)));
        assert_eq!("slli", c.decode_rv32i(enc(MATCH_SLLI, MASK_SLLI)));
        assert_eq!("slt", c.decode_rv32i(enc(MATCH_SLT, MASK_SLT)));
        assert_eq!("slti", c.decode_rv32i(enc(MATCH_SLTI, MASK_SLTI)));
        assert_eq!("sltiu", c.decode_rv32i(enc(MATCH_SLTIU, MASK_SLTIU)));
        assert_eq!("sltu", c.decode_rv32i(enc(MATCH_SLTU, MASK_SLTU)));
        assert_eq!("sra", c.decode_rv32i(enc(MATCH_SRA, MASK_SRA)));
        assert_eq!("srai", c.decode_rv32i(enc(MATCH_SRAI, MASK_SRAI)));
        assert_eq!("srl", c.decode_rv32i(enc(MATCH_SRL, MASK_SRL)));
        assert_eq!("srli", c.decode_rv32i(enc(MATCH_SRLI, MASK_SRLI)));
        assert_eq!("sub", c.decode_rv32i(enc(MATCH_SUB, MASK_SUB)));
        assert_eq!("sw", c.decode_rv32i(enc(MATCH_SW, MASK_SW)));
        assert_eq!("xor", c.decode_rv32i(enc(MATCH_XOR, MASK_XOR)));
        assert_eq!("xori", c.decode_rv32i(enc(MATCH_XORI, MASK_XORI)));
    }

    impl DecodeRv32m for Checker {
        type Item = &'static str;

        fn mul(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "mul"
        }

        fn mulh(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "mulh"
        }

        fn mulhsu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "mulhsu"
        }

        fn mulhu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "mulhu"
        }

        fn div(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "div"
        }

        fn divu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "divu"
        }

        fn rem(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "rem"
        }

        fn remu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "remu"
        }
    }
    ...
    fn decode_rv32im() {
        let mut c = Checker {};
        assert_eq!("div", c.decode_rv32im(enc(MATCH_DIV, MASK_DIV)));
        assert_eq!("divu", c.decode_rv32im(enc(MATCH_DIVU, MASK_DIVU)));
        assert_eq!("mul", c.decode_rv32im(enc(MATCH_MUL, MASK_MUL)));
        assert_eq!("mulh", c.decode_rv32im(enc(MATCH_MULH, MASK_MULH)));
        assert_eq!("mulhsu", c.decode_rv32im(enc(MATCH_MULHSU, MASK_MULHSU)));
        assert_eq!("mulhu", c.decode_rv32im(enc(MATCH_MULHU, MASK_MULHU)));
        assert_eq!("rem", c.decode_rv32im(enc(MATCH_REM, MASK_REM)));
        assert_eq!("remu", c.decode_rv32im(enc(MATCH_REMU, MASK_REMU)));
    }

    impl DecodeRv32c for Checker {
        type Item = &'static str;

        fn c_addi4spn(&mut self, _rdp: crate::Reg, _imm: u32) -> Self::Item {
            "c.addi4spn"
        }

        fn c_lw(&mut self, _rdp: crate::Reg, _rs1p: crate::Reg, _imm: u32) -> Self::Item {
            "c.lw"
        }

        fn c_sw(&mut self, _rs1p: crate::Reg, _rs2p: crate::Reg, _imm: u32) -> Self::Item {
            "c.sw"
        }

        fn c_sub(&mut self, _rdrs1p: crate::Reg, _rs2p: crate::Reg) -> Self::Item {
            "c.sub"
        }

        fn c_xor(&mut self, _rdrs1p: crate::Reg, _rs2p: crate::Reg) -> Self::Item {
            "c.xor"
        }

        fn c_or(&mut self, _rdrs1p: crate::Reg, _rs2p: crate::Reg) -> Self::Item {
            "c.or"
        }

        fn c_and(&mut self, _rdrs1p: crate::Reg, _rs2p: crate::Reg) -> Self::Item {
            "c.and"
        }
        ...
        fn c_andi(&mut self, _rsrs1p: crate::Reg, _imm: u32) -> Self::Item {
            "c.andi"
        }

        fn c_addi(&mut self, _rdrs1n0: crate::Reg, _imm: u32) -> Self::Item {
            "c.addi"
        }

        fn c_li(&mut self, _rd: crate::Reg, _imm: u32) -> Self::Item {
            "c.li"
        }

        fn c_lui(&mut self, _rdn2: crate::Reg, _imm: u32) -> Self::Item {
            "c.lui"
        }
        ...
        fn c_beqz(&mut self, _rs1p: crate::Reg, _imm: u32) -> Self::Item {
            "c.beqz"
        }

        fn c_bnez(&mut self, _rs1p: crate::Reg, _imm: u32) -> Self::Item {
            "c.bnez"
        }

        fn c_jr(&mut self, _rs1n0: crate::Reg) -> Self::Item {
            "c.jr"
        }

        fn c_jalr(&mut self, _rs1n0: crate::Reg) -> Self::Item {
            "c.jalr"
        }
        ...
        fn c_mv(&mut self, _rd: crate::Reg, _rs2n0: crate::Reg) -> Self::Item {
            "c.mv"
        }

        fn c_add(&mut self, _rdrs1: crate::Reg, _rs2n0: crate::Reg) -> Self::Item {
            "c.add"
        }

        fn c_lwsp(&mut self, _rdn0: crate::Reg, _imm: u32) -> Self::Item {
            "c.lwsp"
        }

        fn c_swsp(&mut self, _rs2: crate::Reg, _imm: u32) -> Self::Item {
            "c.swsp"
        }
        ...
        fn c_srli(&mut self, _rdrs1p: crate::Reg, _imm: u32) -> Self::Item {
            "c.srli"
        }

        fn c_srai(&mut self, _rdrs1p: crate::Reg, _imm: u32) -> Self::Item {
            "c.srai"
        }

        fn c_slli(&mut self, _rdrs1n0: crate::Reg, _imm: u32) -> Self::Item {
            "c.slli"
        }
    }
    ...
    fn decode_rv32imc() {
        let mut c = Checker {};
        assert_eq!("c.add", c.decode_rv32imc(enc(MATCH_C_ADD, MASK_C_ADD)));
        assert_eq!("c.addi", c.decode_rv32imc(enc(MATCH_C_ADDI, MASK_C_ADDI)));
        assert_eq!(
            "c.addi16sp",
            c.decode_rv32imc(enc(MATCH_C_ADDI16SP, MASK_C_ADDI16SP))
        );
        assert_eq!(
            "c.addi4spn",
            c.decode_rv32imc(enc(MATCH_C_ADDI4SPN, MASK_C_ADDI4SPN))
        );
        assert_eq!("c.and", c.decode_rv32imc(enc(MATCH_C_AND, MASK_C_AND)));
        assert_eq!("c.andi", c.decode_rv32imc(enc(MATCH_C_ANDI, MASK_C_ANDI)));
        assert_eq!("c.beqz", c.decode_rv32imc(enc(MATCH_C_BEQZ, MASK_C_BEQZ)));
        assert_eq!("c.bnez", c.decode_rv32imc(enc(MATCH_C_BNEZ, MASK_C_BNEZ)));
        assert_eq!(
            "c.ebreak",
            c.decode_rv32imc(enc(MATCH_C_EBREAK, MASK_C_EBREAK))
        );
        assert_eq!("c.j", c.decode_rv32imc(enc(MATCH_C_J, MASK_C_J)));
        assert_eq!("c.jal", c.decode_rv32imc(enc(MATCH_C_JAL, MASK_C_JAL)));
        assert_eq!("c.jalr", c.decode_rv32imc(enc(MATCH_C_JALR, MASK_C_JALR)));
        assert_eq!("c.jr", c.decode_rv32imc(enc(MATCH_C_JR, MASK_C_JR)));
        assert_eq!("c.li", c.decode_rv32imc(enc(MATCH_C_LI, MASK_C_LI)));
        assert_eq!("c.lui", c.decode_rv32imc(enc(MATCH_C_LUI, MASK_C_LUI)));
        assert_eq!("c.lw", c.decode_rv32imc(enc(MATCH_C_LW, MASK_C_LW)));
        assert_eq!("c.lwsp", c.decode_rv32imc(enc(MATCH_C_LWSP, MASK_C_LWSP)));
        assert_eq!("c.mv", c.decode_rv32imc(enc(MATCH_C_MV, MASK_C_MV)));
        assert_eq!("c.nop", c.decode_rv32imc(enc(MATCH_C_NOP, MASK_C_NOP)));
        assert_eq!("c.or", c.decode_rv32imc(enc(MATCH_C_OR, MASK_C_OR)));
        assert_eq!("c.slli", c.decode_rv32imc(enc(MATCH_C_SLLI, MASK_C_SLLI)));
        assert_eq!("c.srai", c.decode_rv32imc(enc(MATCH_C_SRAI, MASK_C_SRAI)));
        assert_eq!("c.srli", c.decode_rv32imc(enc(MATCH_C_SRLI, MASK_C_SRLI)));
        assert_eq!("c.sub", c.decode_rv32imc(enc(MATCH_C_SUB, MASK_C_SUB)));
        assert_eq!("c.sw", c.decode_rv32imc(enc(MATCH_C_SW, MASK_C_SW)));
        assert_eq!("c.swsp", c.decode_rv32imc(enc(MATCH_C_SWSP, MASK_C_SWSP)));
        assert_eq!("c.xor", c.decode_rv32imc(enc(MATCH_C_XOR, MASK_C_XOR)));
    }

    impl DecodeRv32f for Checker {
        type Item = &'static str;

        fn flw(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _iimm: u32) -> Self::Item {
            "flw"
        }

        fn fsw(&mut self, _rs1: crate::Reg, _rs2: crate::Reg, _simm: u32) -> Self::Item {
            "fsw"
        }

        fn fsqrt_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rm: u32) -> Self::Item {
            "fsqrt.s"
        }

        fn fcvt_w_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rm: u32) -> Self::Item {
            "fcvt.w.s"
        }

        fn fcvt_wu_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rm: u32) -> Self::Item {
            "fcvt.wu.s"
        }

        fn fcvt_s_w(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rm: u32) -> Self::Item {
            "fcvt.s.w"
        }

        fn fcvt_s_wu(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rm: u32) -> Self::Item {
            "fcvt.s.wu"
        }
        ...
        fn fmv_x_w(&mut self, _rd: crate::Reg, _rs1: crate::Reg) -> Self::Item {
            "fmv.x.w"
        }

        fn fmv_w_x(&mut self, _rd: crate::Reg, _rs1: crate::Reg) -> Self::Item {
            "fmv.w.x"
        }

        fn fclass_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg) -> Self::Item {
            "fclass.s"
        }

        fn fsgnj_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "fsgnj.s"
        }

        fn fmin_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "fmin.s"
        }

        fn fle_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "fle.s"
        }

        fn fsgnjn_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "fsgnjn.s"
        }

        fn fmax_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "fmax.s"
        }

        fn flt_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "flt.s"
        }

        fn fsgnjx_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "fsgnjx.s"
        }

        fn feq_s(&mut self, _rd: crate::Reg, _rs1: crate::Reg, _rs2: crate::Reg) -> Self::Item {
            "feq.s"
        }
    }
    ...
    fn decode_rv32imf() {
        let mut c = Checker {};
        assert_eq!("fadd.s", c.decode_rv32imf(enc(MATCH_FADD_S, MASK_FADD_S)));
        assert_eq!(
            "fclass.s",
            c.decode_rv32imf(enc(MATCH_FCLASS_S, MASK_FCLASS_S))
        );
        assert_eq!(
            "fcvt.s.w",
            c.decode_rv32imf(enc(MATCH_FCVT_S_W, MASK_FCVT_S_W))
        );
        assert_eq!(
            "fcvt.s.wu",
            c.decode_rv32imf(enc(MATCH_FCVT_S_WU, MASK_FCVT_S_WU))
        );
        assert_eq!(
            "fcvt.w.s",
            c.decode_rv32imf(enc(MATCH_FCVT_W_S, MASK_FCVT_W_S))
        );
        assert_eq!(
            "fcvt.wu.s",
            c.decode_rv32imf(enc(MATCH_FCVT_WU_S, MASK_FCVT_WU_S))
        );
        assert_eq!("fdiv.s", c.decode_rv32imf(enc(MATCH_FDIV_S, MASK_FDIV_S)));
        assert_eq!("feq.s", c.decode_rv32imf(enc(MATCH_FEQ_S, MASK_FEQ_S)));
        assert_eq!("fle.s", c.decode_rv32imf(enc(MATCH_FLE_S, MASK_FLE_S)));
        assert_eq!("flt.s", c.decode_rv32imf(enc(MATCH_FLT_S, MASK_FLT_S)));
        assert_eq!("flw", c.decode_rv32imf(enc(MATCH_FLW, MASK_FLW)));
        assert_eq!(
            "fmadd.s",
            c.decode_rv32imf(enc(MATCH_FMADD_S, MASK_FMADD_S))
        );
        assert_eq!("fmax.s", c.decode_rv32imf(enc(MATCH_FMAX_S, MASK_FMAX_S)));
        assert_eq!("fmin.s", c.decode_rv32imf(enc(MATCH_FMIN_S, MASK_FMIN_S)));
        assert_eq!(
            "fmsub.s",
            c.decode_rv32imf(enc(MATCH_FMSUB_S, MASK_FMSUB_S))
        );
        assert_eq!("fmul.s", c.decode_rv32imf(enc(MATCH_FMUL_S, MASK_FMUL_S)));
        assert_eq!(
            "fmv.w.x",
            c.decode_rv32imf(enc(MATCH_FMV_W_X, MASK_FMV_W_X))
        );
        assert_eq!(
            "fmv.x.w",
            c.decode_rv32imf(enc(MATCH_FMV_X_W, MASK_FMV_X_W))
        );
        assert_eq!(
            "fnmadd.s",
            c.decode_rv32imf(enc(MATCH_FNMADD_S, MASK_FNMADD_S))
        );
        assert_eq!(
            "fnmsub.s",
            c.decode_rv32imf(enc(MATCH_FNMSUB_S, MASK_FNMSUB_S))
        );
        assert_eq!(
            "fsgnj.s",
            c.decode_rv32imf(enc(MATCH_FSGNJ_S, MASK_FSGNJ_S))
        );
        assert_eq!(
            "fsgnjn.s",
            c.decode_rv32imf(enc(MATCH_FSGNJN_S, MASK_FSGNJN_S))
        );
        assert_eq!(
            "fsgnjx.s",
            c.decode_rv32imf(enc(MATCH_FSGNJX_S, MASK_FSGNJX_S))
        );
        assert_eq!(
            "fsqrt.s",
            c.decode_rv32imf(enc(MATCH_FSQRT_S, MASK_FSQRT_S))
        );
        assert_eq!("fsub.s", c.decode_rv32imf(enc(MATCH_FSUB_S, MASK_FSUB_S)));
        assert_eq!("fsw", c.decode_rv32imf(enc(MATCH_FSW, MASK_FSW)));
    }
}
