...
pub trait DispatchRv32i {
    type Item;

    /// Decodes the input word to an RV32I instruction and dispatches it to a handler.
    fn dispatch(&mut self, code: u32) -> <Self as HandleRv32i>::Item
    where
        Self: HandleRv32i;
}

impl<T, U> DispatchRv32i for T
where
    T: HandleRv32i + HandleRv32i<Item = U>,
{
    type Item = U;
...
pub trait DispatchRv32im {
    type Item;

    /// Decodes the input word to an RV32IM instruction and dispatches it to a handler.
    fn dispatch(&mut self, code: u32) -> <Self as HandleRv32i>::Item
    where
        Self: HandleRv32i + HandleRv32m;
}

impl<T, U> DispatchRv32im for T
where
    T: HandleRv32i + HandleRv32i<Item = U> + HandleRv32m + HandleRv32m<Item = U>,
{
    type Item = U;
...
pub trait DispatchRv32ic {
    type Item;

    /// Decodes the input word to an RV32IC instruction and dispatches it to a handler.
    fn dispatch(&mut self, code: u32) -> <Self as HandleRv32i>::Item
    where
        Self: HandleRv32i + HandleRv32c;
}

impl<T, U> DispatchRv32ic for T
where
    T: HandleRv32i + HandleRv32i<Item = U> + HandleRv32c + HandleRv32c<Item = U>,
{
    type Item = U;
...
pub trait DispatchRv32imc {
    type Item;

    /// Decodes the input word to an RV32IMC instruction and dispatches it to a handler.
    fn dispatch(&mut self, code: u32) -> <Self as HandleRv32i>::Item
    where
        Self: HandleRv32i + HandleRv32m + HandleRv32c;
}

impl<T, U> DispatchRv32imc for T
where
    T: HandleRv32i
        + HandleRv32i<Item = U>
        + HandleRv32m
...
pub trait DispatchRv32imf {
    type Item;

    /// Decodes the input word to an RV32IMF instruction and dispatches it to a handler.
    fn dispatch(&mut self, code: u32) -> <Self as HandleRv32i>::Item
    where
        Self: HandleRv32i + HandleRv32m + HandleRv32f;
}

impl<T, U> DispatchRv32imf for T
where
    T: HandleRv32i
        + HandleRv32i<Item = U>
        + HandleRv32m
...
pub trait DispatchRv32imfc {
    type Item;

    /// Decodes the input word to an RV32IMFC instruction and dispatches it to a handler.
    fn dispatch(&mut self, code: u32) -> <Self as HandleRv32i>::Item
    where
        Self: HandleRv32i + HandleRv32m + HandleRv32f + HandleRv32c;
}

impl<T, U> DispatchRv32imfc for T
where
    T: HandleRv32i
        + HandleRv32i<Item = U>
        + HandleRv32m
...
#[cfg(test)]
mod test {
    use super::{HandleRv32c, HandleRv32f, HandleRv32i, HandleRv32m};
    use crate::reg::Reg;
