...
def generate_bitmask_code(specs: List[Spec], extensions: str):
    ...
    short_bounds = " + ".join(f"Rv32{x}" for x in extensions)
    full_bounds = "\n        + ".join(f"Rv32{x}Handler\n        + Rv32{x}Handler<Item = U>" for x in extensions)
    preamble = f"""\
...
"""
    ...
    for k, v in ordered_patterns.items():
        ...
        for value, operator, operands in v:
            ...
            print(f"            0x{match_value:0{width}x} => return self.{operator}{operands},")
        print("            _ => {}")
        print("        }")
    print(postamble)
