...
def generate_rust_code_v3(d, level=0):
    lut = {
        "bimm12hi_bimm12lo_rs1_rs2": "(c.bimmediate(), c.rs1(), c.rs2())",
        "fm_pred_rd_rs1_succ": "(c.fm(), c.rd(), c.rs1())",
        "imm12_rd_rs1":  "(c.iimmediate(), c.rd(), c.rs1())",
        "imm12hi_imm12lo_rs1_rs2": "(c.simmediate(code), c.rs1(), c.rs2())",
        "imm20_rd": "(c.uimmediate(), c.rd())",
        "jimm20_rd": "(c.jimmediate(), c.rd())",
        "no_args": "()",
    ...
    if level == 0:
        print(
            "pub fn decode<T>(decoder: &mut (impl Decoder + Decoder<Item = T>), code: u32) -> T {")
        print(f"{indent}// This function is generated by make_decoder.py. Do not edit.")

    for k, v in sorted(d.items()):
        hi, lo = k
        print(f"{indent}match bits(code, {hi}, {lo}) {{")
        for bit_pattern, content in sorted(v.items()):
            print(f"{i2}0x{bit_pattern:x} => ", end="")
    ...
    if level == 0:
        print(f"{indent}// Illegal instruction.")
        print(f"{indent}return decoder.trap(IllegalInstruction, code);")
        print("}")
