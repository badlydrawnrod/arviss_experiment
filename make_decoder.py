...
def generate_rust_code(d, level=0):
    ...
    if level == 0:
        print("fn decode<T>(decoder: &mut (impl Decoder + Decoder<Item = T>), ins: u32) -> T {")
        print(f"{indent}// This function is generated by make_decoder.py. Do not edit.")

    for k, v in sorted(d.items()):
        hi, lo = k
        print(f"{indent}match bits(ins, {hi}, {lo}) {{")
        for bit_pattern, content in sorted(v.items()):
            ...
            if isinstance(content, dict):
                ...
                if len(arg_part) == 0:
                    arg_part = "no_args"
                function_name = f"gen_{arg_part}"
                print(
                    f"return decoder.{function_name}({opcode}, ins), // {content[0]}")
        print(f"{i2}_ => {{}}")
        print(f"{indent}}}")

    if level == 0:
        print(f"{indent}// Illegal instruction.")
        print(f"{indent}return decoder.gen_trap(ExecIllegalInstruction, ins);")
        print("}")
